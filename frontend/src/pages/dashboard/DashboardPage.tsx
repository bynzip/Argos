...
const StatCard = ({ label, value, icon: Icon, color, link, linkText }: StatCardProps) => {
  const colorMap = {
  ...
  return (
    <div className="bg-white border border-[var(--gray-200)] rounded-xl p-5 shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] transition-all group">
      <div className="flex justify-between items-start mb-4">
        <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110", colorMap[color])}>
          <Icon size={22} />
        </div>
        {link && (
          <Link to={link} className="text-[11px] font-bold text-[var(--color-brand-blue)] hover:underline uppercase tracking-wider">
            {linkText || 'Ver más'}
          </Link>
        )}
      </div>
      <div>
        <p className="text-[11px] font-bold text-[var(--gray-400)] uppercase tracking-[0.05em] mb-1">{label}</p>
        <p className="text-[28px] font-extrabold text-[var(--gray-800)] leading-none">{value}</p>
      </div>
    </div>
  );
};

const SimpleBarChart = ({ data, title, icon: Icon, colorClass = "bg-[var(--color-brand-blue)]" }: { data: Record<string, number>, title: string, icon: any, colorClass?: string }) => {
          ...
          entries.map(([key, val]) => (
            <div key={key} className="space-y-1.5">
              <div className="flex justify-between items-end">
                ...
                <span className="text-[13px] font-black text-[var(--gray-800)]">{val}</span>
              </div>
          ...
          <div className="bg-[var(--gradient-brand)] rounded-xl p-6 text-white shadow-[var(--shadow-md)] relative overflow-hidden h-full flex flex-col justify-center">
             ...
             <div className="relative z-10">
               ...
               <p className="text-blue-50 text-sm leading-relaxed mb-6">
                 Gestión profesional para servicios técnicos y talleres.
               </p>
               <div className="flex items-center gap-2 text-[10px] font-black bg-white/20 w-fit px-3 py-1.5 rounded-full uppercase tracking-widest">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white border border-[var(--gray-200)] rounded-xl shadow-[var(--shadow-sm)] overflow-hidden">
            <div className="px-6 py-4 border-b border-[var(--gray-100)] flex justify-between items-center bg-[var(--gray-50)]">
              <h2 className="text-[14px] font-bold text-[var(--gray-800)] uppercase tracking-tight">Actividad Reciente</h2>
              <Link to="/tickets" className="text-[11px] font-bold text-[var(--color-brand-blue)] hover:underline uppercase tracking-wider">VER TODOS</Link>
            </div>
                  ...
                  {recentTickets && recentTickets.length > 0 ? (
                    (recentTickets as Ticket[]).map((ticket) => (
                      <tr key={ticket.id} className="hover:bg-[var(--gray-50)] transition-colors">
                        <td className="px-6 py-4 text-sm font-bold text-[var(--color-brand-blue)]">{ticket.folio}</td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-[var(--gray-700)]">{ticket.customer?.nombre || '-'}</span>
                            <span className="text-[11px] font-medium text-[var(--gray-400)]">{ticket.device?.marca} {ticket.device?.modelo}</span>
                          </div>
                        </td>
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-10 text-center text-[var(--gray-400)] italic text-sm">
          ...
          {data.metrics.low_stock_alerts > 0 && (
            <div className="bg-[var(--color-danger-bg)] border border-[var(--color-danger-border)] rounded-xl p-6 shadow-sm">
              <div className="flex items-center gap-3 text-[var(--color-danger)] mb-4">
                ...
                <h4 className="font-extrabold text-[15px] uppercase tracking-tight">Alertas de Stock</h4>
              </div>
              <p className="text-sm text-[var(--color-danger)] mb-6 leading-relaxed font-medium">
                Hay **{data.metrics.low_stock_alerts}** productos que requieren reposición inmediata.
              </p>
              <Link to="/inventory">
                <Button variant="danger" className="w-full font-bold">Gestionar Inventario</Button>
              </Link>
            </div>
          )}
          
          <div className="bg-white border border-[var(--gray-200)] rounded-xl p-6">
            <h4 className="text-[13px] font-bold text-[var(--gray-800)] uppercase tracking-wider mb-4 border-b border-[var(--gray-100)] pb-3">Resumen de Turno</h4>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-xs font-medium text-[var(--gray-500)]">Iniciado por</span>
                <span className="text-xs font-bold text-[var(--gray-800)]">{user?.nombre}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs font-medium text-[var(--gray-500)]">Rol asignado</span>
                <span className="text-xs font-bold text-[var(--color-brand-blue)] bg-[var(--color-info-bg)] px-2 py-0.5 rounded-full">{user?.role}</span>
              </div>
            </div>
          </div>
        </div>
